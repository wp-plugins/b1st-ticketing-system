...
class users_model extends CI_Model
{
	...
	{
		$query=$this->db->query("select responder_time_duration from ".$this->db->dbprefix."responder_time_duration where userid='".$userid."' order by (currentdate) DESC limit 0,1");
		$r = $query->row();
		if(!empty($r))
		{
			
			return $r->responder_time_duration;
		}
		else
		{
			return "";
